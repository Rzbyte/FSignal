...
**[🚀 LIVE DASHBOARD](https://fsignal-production.up.railway.app)** ·
[`/health`](https://fsignal-production.up.railway.app/health) ·
[`/ledger`](https://fsignal-production.up.railway.app/ledger)

> **Why these are open.** The dashboard and `/ledger` carry the companies themselves, and
> on a public deployment that is the pipeline this bot exists to give one person an edge
> in. They are open anyway, deliberately, while the project is under review: a claim you
> cannot check is not evidence, and asking a reviewer to take the precision numbers on
> trust would cost more than the exposure does.
>
> Closing them is one variable — `DASHBOARD_TOKEN` gates all three behind
> `Authorization: Bearer …` or `?token=…`, and `/health` reports
> `lead_endpoints_protected` so the posture is never a guess. It is tested both ways in
> `tests/test_endpoint_access.py`. `/health` and the Pond endpoints are never gated: the
> first carries counts rather than companies, the second is how Pond health-checks the
> agent.
...
live at [`/ledger`](https://fsignal-production.up.railway.app/ledger), and committed as
[`evidence/raw/ledger.json`](evidence/raw/ledger.json) so the numbers stay checkable after
the deployment stops being public.
...
| Lead-endpoint auth | **implemented, deliberately off** | `DASHBOARD_TOKEN` gates the dashboard, `/ledger` and timelines. Left unset while under review so the evidence stays checkable; verified working in both postures |
...
- **The lead endpoints are open on this deployment, on purpose and only for now.** The
  dashboard, `/ledger` and `/signals/{id}/timeline` carry the companies themselves, so a
  public deployment publishes the pipeline the operator is relying on being alone in
  having. That is a real cost, taken knowingly: while the project is under review, a
  reviewer being able to open the ledger is worth more than the exposure. Set
  `DASHBOARD_TOKEN` and all three answer `401` without `Authorization: Bearer …` or
  `?token=…` — the second because these are pages a person opens in a browser, and a
  browser cannot send a header. `/health` reports `lead_endpoints_protected` either way.
  Anyone running this for real should set it on day one.
